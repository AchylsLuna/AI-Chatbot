...
const sidebarItems: SidebarItem[] = [
  ...
  { key: 'staff_management', label: 'Staff Management', icon: 'hospital' },
...
const fallbackUserItems: UserManagementItem[] = [
  {
    id: 'RES-10021',
...
const fallbackStaffItems: StaffManagementItem[] = [
  {
    id: 'STF-001',
    name: 'Dr. Mara Santos',
    role: 'Admin / Doctor',
...
const userStatusChipClass = (status: UserManagementItem['latestStatus']) => {
...
const staffStatusChipClass = (status: StaffManagementItem['status']) => {
...
const isPrivilegedRole = (role?: AuthSession['user']['role'] | null) =>
...
const DoctorDashboardPage = ({
  ...
  const [staffManagementPage, setStaffManagementPage] = useState(1)
  ...
  const setSection = (next: StaffSidebarSection) => {
    if (next !== 'dashboard') {
      setSearchQuery('')
    }
    ...
    if (next === 'staff_management') {
      setStaffManagementPage(1)
    }
  ...
  const userManagementItems = useMemo<UserManagementItem[]>(() => {
    ...
    const perUser = new Map<string, UserManagementItem>()
  ...
  const staffManagementItems = useMemo<StaffManagementItem[]>(() => {
    const items = [...fallbackStaffItems]

    if (authUser && isPrivilegedRole(authUser.role)) {
      const statusText: StaffManagementItem['status'] =
    ...
    const unique = new Map<string, StaffManagementItem>()
    for (const item of items) {
  ...
  const userManagementTotalPages = useMemo(
    () => Math.max(1, Math.ceil(userManagementItems.length / PAGINATION_PAGE_SIZE)),
    [userManagementItems.length]
  )

  const staffManagementTotalPages = useMemo(
    () => Math.max(1, Math.ceil(staffManagementItems.length / PAGINATION_PAGE_SIZE)),
    [staffManagementItems.length]
  )

  useEffect(() => {
    setUserManagementPage((previous) => Math.min(previous, userManagementTotalPages))
  }, [userManagementTotalPages])

  useEffect(() => {
    setStaffManagementPage((previous) => Math.min(previous, staffManagementTotalPages))
  }, [staffManagementTotalPages])
  ...
  const renderOperationsBoard = () => {
    const pagedReservations = getPageSlice(filteredReservations, appointmentsPage)
  ...
  const renderUserManagement = () => {
    const pagedUsers = getPageSlice(userManagementItems, userManagementPage)

    return (
      <section className="space-y-3">
        {userManagementItems.length === 0 ? (
          <article className="reference-card p-5">
            <h2 className="reference-section-title">No user records available</h2>
            <p className="reference-widget-subtle mt-2">
              User booking activity will appear here after appointments are submitted.
            </p>
          </article>
        ) : (
          <article className="reference-card p-4">
                    ...
                    return (
                      <tr key={`${item.id}-${item.displayName}`}>
                        <td className="px-3 py-3 align-top">
                          ...
                          <p className="text-xs text-[color:var(--agent-muted)]">{displayId}</p>
                        </td>
                        ...
                        <td className="px-3 py-3 text-[color:var(--agent-muted)]">
                          {formatDateTime(item.latestActivity)}
                        </td>
                        <td className="px-3 py-3">
                          <span
                            className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${userStatusChipClass(item.latestStatus)}`}
                          >
                            {item.latestStatus}
                          </span>
                        </td>
        ...
        <PaginationControls
          ...
          totalItems={userManagementItems.length}
          pageSize={PAGINATION_PAGE_SIZE}
          maxPageButtons={MAX_PAGE_BUTTONS}
  ...
  const renderStaffManagement = () => {
    const pagedStaff = getPageSlice(staffManagementItems, staffManagementPage)

    return (
      <section className="space-y-3">
        {staffManagementItems.length === 0 ? (
          <article className="reference-card p-5">
            <h2 className="reference-section-title">No staff records available</h2>
            <p className="reference-widget-subtle mt-2">
              Staff assignments will appear here when admin and nurse users are active.
            </p>
          </article>
        ) : (
          <article className="reference-card p-4">
              ...
              <table className="min-w-full divide-y divide-[color:var(--card-border)] text-sm">
                ...
                <tbody className="divide-y divide-[color:var(--card-border)]">
                  {pagedStaff.map((item) => (
                    <tr key={item.id}>
                      <td className="px-3 py-3 align-top">
                        <p className="font-semibold text-[color:var(--agent-ink)]">{item.name}</p>
                        <p className="text-xs text-[color:var(--agent-muted)]">{item.id}</p>
                      </td>
                      <td className="px-3 py-3 text-[color:var(--agent-muted)]">{item.role}</td>
                      <td className="px-3 py-3 text-[color:var(--agent-muted)]">{item.workspace}</td>
                      <td className="px-3 py-3 text-[color:var(--agent-muted)]">{item.lastAction}</td>
                      <td className="px-3 py-3">
                        <span
                          className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${staffStatusChipClass(item.status)}`}
                        >
                          {item.status}
                        </span>
                      </td>
        ...
        <PaginationControls
          currentPage={staffManagementPage}
          totalItems={staffManagementItems.length}
          pageSize={PAGINATION_PAGE_SIZE}
          maxPageButtons={MAX_PAGE_BUTTONS}
          onPageChange={setStaffManagementPage}
        />
      </section>
    )
  }
  ...
  const sectionTitleMap: Record<StaffSidebarSection, string> = {
    dashboard: 'Dashboard',
    appointments: 'Appointments',
    user_management: 'User Management',
    staff_management: 'Staff Management',
    saved: 'Saved items',
    wallet: 'Operations resources',
  ...
  return (
    <WorkspaceCanvas>
      <div className="w-full overflow-x-auto">
        <div className="reference-shell h-screen min-w-[1080px]">
          <Sidebar
            variant="reference"
          ...
          <section className="reference-main h-screen overflow-y-auto px-4 pb-10 pt-5 sm:px-6 lg:px-8">
            ...
            {activeSection === 'user_management' ? renderUserManagement() : null}
            {activeSection === 'staff_management' ? renderStaffManagement() : null}
