...
type DoctorProfileSettingsSectionProps = {
  authUser: AuthSession['user'] | null
  sessionStatus: string
}

const DoctorProfileSettingsSection = ({ authUser, sessionStatus }: DoctorProfileSettingsSectionProps) => {
  return (
    <section className="space-y-4">
      <article className={`${workspacePanelClass} p-5`}>
        ...
        <p className={`mt-2 text-sm ${workspaceMutedTextClass}`}>
          Manage profile identity for the doctor workspace.
        </p>
      ...
      <ProfileAvatarSettingsCard
        ...
        title="Profile photo"
        description="Upload or replace your profile photo for doctor workspace identity."
      />
    </section>
  )
}
