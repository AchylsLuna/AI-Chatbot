  ...
  getUserSettings: async (): Promise<UserSettings> => {
    ...
    const rawSettings =
      ...
      typeof (payload as Record<string, unknown>).settings === 'object'
        ? ((payload as Record<string, unknown>).settings as Record<string, unknown>)
        : {}
    const notificationsRaw =
      rawSettings.notifications && typeof rawSettings.notifications === 'object'
        ? (rawSettings.notifications as Record<string, unknown>)
        : {}

    return {
      theme: rawSettings.theme === 'dark' ? 'dark' : defaultUserSettings.theme,
      notifications: {
        email:
          typeof notificationsRaw.email === 'boolean'
            ? notificationsRaw.email
            : defaultUserSettings.notifications.email,
        sms:
          typeof notificationsRaw.sms === 'boolean'
            ? notificationsRaw.sms
    ...
    for (const dayKey of Object.keys(days) as Array<keyof DoctorScheduleDays>) {
      ...
      weeklySlots[dayKey] = daySlotsRaw.map((item) => {
        ...
        const session: 'morning' | 'afternoon' =
          sessionRaw === 'afternoon' ? 'afternoon' : 'morning'
        return {
          startIso: String(raw.startIso || ''),
          endIso: String(raw.endIso || ''),
          label: String(raw.label || ''),
