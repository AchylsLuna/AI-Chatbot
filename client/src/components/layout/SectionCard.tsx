      ...
      <div className="border-b border-[color:var(--card-border)] px-5 py-5 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            ...
            <h2 className={`mt-1 text-lg font-semibold sm:text-xl ${pageHeadingTextClass}`}>{title}</h2>
            {description ? (
              <p className={`mt-2 text-sm ${pageMutedTextClass}`}>{description}</p>
            ) : null}
          </div>
        ...
        {toolbar ? (
          <div className="mt-4 rounded-2xl border border-[color:var(--card-border)] bg-[color:var(--agent-surface-strong)] px-4 py-3">
            {toolbar}
          </div>
        ) : null}
      </div>
